class AdminController < ApplicationController
  ...
  def index
    ...
    if @revenue_by_day.count < 7
      ...
      current_day_index = (days_of_week.index(current_day) + 1) % days_of_week.length
      next_day_index = (current_day_index + 1) % days_of_week.length

      ordered_day_with_current_last = days_of_week[next_day_index..-1] + days_of_week[0...next_day_index]
      complete_ordered_array_with_current_last = ordered_day_with_current_last.map { |day| [ day, data_hash.fetch(day, 0) ] }
